...
export default registerAs('app', () => ({
  ...
  defaultLlmModel: process.env.DEFAULT_LLM_MODEL ?? 'deepseek-v4-pro',
  openaiApiKey: process.env.OPENAI_API_KEY ?? '',
  openaiBaseUrl: process.env.OPENAI_BASE_URL ?? '',
  deepseekApiKey: process.env.DEEPSEEK_API_KEY ?? '',
  deepseekBaseUrl: process.env.DEEPSEEK_BASE_URL ?? 'https://api.deepseek.com',
  deepseekReasoningEffort: process.env.DEEPSEEK_REASONING_EFFORT ?? 'high',
  deepseekThinkingType: process.env.DEEPSEEK_THINKING_TYPE ?? 'enabled',
  autoCharacterModel: process.env.AUTO_CHARACTER_MODEL ?? process.env.DEFAULT_LLM_MODEL ?? 'deepseek-v4-pro',
  ...
  aliyunSmsSignName: process.env.ALIYUN_SMS_SIGN_NAME ?? '天津易比达教育科技',
  aliyunSmsTemplateCode: process.env.ALIYUN_SMS_TEMPLATE_CODE ?? 'SMS_486090102',
